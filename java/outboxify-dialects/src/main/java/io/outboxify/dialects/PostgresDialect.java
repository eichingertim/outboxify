...
/**
 * PostgreSQL 9.5+ dialect supporting FOR UPDATE SKIP LOCKED, LIMIT,
 * CURRENT_TIMESTAMP, and Partial Indexes (WHERE status IN ('NEW', 'FAILED')).
 */
public class PostgresDialect implements DatabaseDialect {
    ...
    public String getStaleThresholdExpression(int seconds) {
        return "CURRENT_TIMESTAMP - (" + seconds + " * INTERVAL '1 SECOND')";
    }
    ...
    public String buildReapStaleQuery(String tableName, ColumnMapping columns, int timeoutSeconds, int maxRetries) {
        return "UPDATE " + tableName +
                ...
                " AND " + columns.getUpdatedAt() + " < (CURRENT_TIMESTAMP - (" + timeoutSeconds + " * INTERVAL '1 SECOND'))";
    }
