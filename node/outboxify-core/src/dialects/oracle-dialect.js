...
class OracleDialect extends DatabaseDialect {
  ...
  getStaleThresholdExpression(seconds) {
    return `SYSTIMESTAMP - NUMTODSINTERVAL(${Number(seconds)}, 'SECOND')`;
  }
  ...
  buildReapStaleQuery(tableName, columns, timeoutSeconds, maxRetries) {
    return `UPDATE ${tableName} ` +
      ...
      `${columns.updatedAt} = SYSTIMESTAMP ` +
      `WHERE ${columns.status} = 'PROCESSING' ` +
      `AND ${columns.updatedAt} < (SYSTIMESTAMP - NUMTODSINTERVAL(${Number(timeoutSeconds)}, 'SECOND'))`;
  }

  getSparseIndexDdl(tableName, indexName, columns) {
    return `CREATE INDEX ${indexName} ON ${tableName} (` +
      `CASE WHEN ${columns.status} IN ('NEW', 'FAILED') THEN ${columns.status} ELSE NULL END, ` +
      `${columns.createdAt})`;
  }
}
